...
//		2E3F85E  03 04 20 03 00 FC 5B 00 00 B1 BE B5 D8 B4 C5 C5 CC 00 4E 54 46 53 00
	...
	/*	CString  strFileSystem;
		strFileSystem = Travel + i;
		m_ControlList_Client.SetItemText(iItem, 1, strFileSystem);*/
		i += strlen(Travel + i) + 1;
...
VOID CFileManagerDlg::SendFileData()
{
	...
	if (m_ulCounter == m_OperatingFileLength||m_bIsStop)  // || pFileSize->dwSizeLow == -1 || m_bIsStop)     //pFileSize->dwSizeLow == -1  是对方选择了跳过    m_nCounter == m_nOperatingFileLength  完成当前的传输
	{
...
/*	int nItem = m_list_remote.GetSelectionMark();
	...
	m_strSourFileFullPath = m_Remote_Upload_Job.GetHead();    //获得第一个任务的名称   D:\1.txt  D:\
	...
	hFile = CreateFile(m_strSourFileFullPath.GetBuffer(0), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);   //获得要发送文件的大小
	if (hFile == INVALID_HANDLE_VALUE)
	{
		return FALSE;
	}
...
/*		BYTE	lpPacket[MAX_PATH+10];
		lpPacket[0] = COMMAND_COMPRESS;
		memcpy(lpPacket + 1,pwszCommandLine, cchCmdLineSize);
		m_iocpServer->Send(m_pContext, lpPacket, cchCmdLineSize+1);*/
	}
