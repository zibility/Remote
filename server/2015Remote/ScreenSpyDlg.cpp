...
/*	ULONG	ulOldCursorIndex = m_bCursorIndex;
	...
	if (ulOldCursorIndex != m_bCursorIndex)
	{
		...
		if (m_bIsCtrl && !m_bIsTraceCursor)  //m_bIsCtrl    m_bIsTraceCursor
		{
			...
			//设置鼠标样式
			//hIco hion  hbuuton   hli      kdjfkjdf= loadkdjfk（afgmakekdjfkdf（dkfj））setioc(句柄)  索引
...
void CScreenSpyDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) //npos 当前滚动轴位置
{
SCROLLINFO si;
int	i;
...
i = max(i, si.nMin);  //0      
i = min(i, (int)(si.nMax - si.nPage + 1));//i = 0    防止出界   //1080    900
